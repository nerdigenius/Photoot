	...
	vpb_multiple_file_uploaders.prototype.vasPLUS = function(file,file_counter)
	{
		if(typeof file[file_counter] != undefined && file[file_counter] != '')
		{
			...
			else
			{
				...
				var custom_q_id = $('#custom_q_id').val();
				var REURL = BASE_URL+'frontend/success_alert_custom_order/'+custom_q_id;
...
function vpb_multiple_file_uploaderss(vpb_configuration_settings)
{
	...
	vpb_multiple_file_uploaderss.prototype.vasPLUS = function(file,file_counter)
	{
		if(typeof file[file_counter] != undefined && file[file_counter] != '')
		{
			...
			var fixed_id = $('input[name="fixed_id"]').val();
			var user_id = $('input[name="user_id"]').val();
			var try_free_value = $('input[name="try_free_value"]').val();
			...
			else
			{
				...
				dataString.append('fixed_id', fixed_id);
				dataString.append('user_id', user_id);
				dataString.append('try_free_value', try_free_value);
				dataString.append('file', files_name_with_extensions);
					...
					success:function(response) 
					{
						...
						setTimeout(function() {
							...
							if ( response_brought != -1) {
								...
								$("#remove"+ids).html('<div align="center" style="font-family:Verdana, Geneva, sans-serif;font-size:11px;color:gray;">Uploaded</div>');
								// alert('Completed');
								var free_q_id = $('#free_q_id').val();
								var URL = BASE_URL+'frontend/success_alert_free_order/'+free_q_id;
								console.log(URL);
								console.log(free_q_id);
